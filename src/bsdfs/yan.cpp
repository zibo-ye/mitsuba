...
public:
    ...
    Spectrum brdf(Float thetaI, Float thetaR, Float Phi) const
    {
        ...
        if (!m_disableR)
            unscatter += MpUnscatter[0] * _nR->eval(Phi, cosThetaD);
        if (!m_disableTT)
            unscatter += MpUnscatter[1] * _nTT->eval(Phi, cosThetaD);
        if (!m_disableTRT)
            unscatter += MpUnscatter[2] * _nTRT->eval(Phi, cosThetaD);

        Spectrum scatter(0.0f);

        if (!m_disableTTs)
            scatter += MpScatter * _nTTs->eval(Phi, cosThetaD);
        if (!m_disableTRTs)
            scatter += MpScatter * _nTRTs->eval(Phi, cosThetaD);
...
private:
    ...
    {
        return (Float)(std::exp(-(x - mu) * (x - mu) / (2 * sigma * sigma)) / (std::sqrt(2 * M_PI) * std::abs(sigma)));
    }
